...
import { weddingData as staticData } from './data.js'
...
import RohitAndManpreetHero from './components/RohitAndManpreetHero.jsx'
import RohitAndManpreetStory from './components/RohitAndManpreetStory.jsx'
import RohitAndManpreetStoryText from './components/RohitAndManpreetStoryText.jsx'
import RohitAndManpreetVenue from './components/RohitAndManpreetVenue.jsx'
...
export default function CustomRoyalHeirloomRohitAndManpreet({ savedData, groupSlug: propGroupSlug }) {
  ...
  // ── Payment / Watermark ────────────────────────────────────────
  const isPaid = (savedData?.paymentStatus === 'PAID') || Boolean(draftData?.paymentStatus === 'PAID')
  const showWatermark = false; // User requested removing watermark

  // ── Data resolution (prioritize savedData/draftData, fallback to staticData) ────────────
  const groomName = (savedData ? (savedData.coupleData?.groomName || savedData.groomName) : draftData?.groomName) || staticData?.heroData?.groomName || 'Rohan'
  const brideName = (savedData ? (savedData.coupleData?.brideName || savedData.brideName) : draftData?.brideName) || staticData?.heroData?.brideName || 'Ananya'
  
  const groomFamily = staticData?.heroData?.groomParents
  const brideFamily = staticData?.heroData?.brideParents
  ...
  const weddingDateStr = (() => {
    ...
    return staticData?.heroData?.weddingDate || '28'
  })()
  const weddingMonth = (() => {
    ...
    return staticData?.heroData?.weddingMonth || 'November'
  })()
  const weddingYear = (() => {
    ...
    return staticData?.heroData?.weddingYear || '2026'
  })()
  const weddingTime = (savedData ? (savedData.heroData?.weddingTime || savedData.weddingTime) : draftData?.weddingTime) || staticData?.heroData?.weddingTime || ''

  // Derive computed values from date components
  const eventDateObj = useMemo(() => {
    // If the date string has " & ", just parse the first part to get a valid Date object for calculations
    const cleanDate = weddingDateStr.split(' & ')[0].replace(/[^0-9]/g, '')
    const d = new Date(`${weddingMonth} ${cleanDate}, ${weddingYear}`)
  ...
  // Venue data
  const venueTitle = (savedData ? (savedData.venueData?.mahalName || savedData.mahalName) : draftData?.mahalName) || staticData?.venue?.venueName || 'The Taj Mahal Palace'
  const venueAddress = (savedData ? (savedData.venueData?.venueAddress || savedData.venueAddress) : draftData?.venueAddress) ?? staticData?.venue?.venueLine1 ?? 'Apollo Bunder, Colaba'
  const venueCity = (savedData ? (savedData.venueData?.venueCity || savedData.venueCity) : draftData?.venueCity) ?? staticData?.venue?.venueLine2 ?? 'Mumbai'
  const venueState = (savedData ? (savedData.venueData?.state || savedData.state) : draftData?.state) ?? ''
  const fullAddress = [venueAddress, venueCity, venueState].filter(Boolean).join(', ')
  const mapUrl = (savedData ? (savedData.venueData?.mapLink || savedData.mapLink) : draftData?.mapLink) || staticData?.venue?.mapUrl || `https://maps.google.com/?q=${encodeURIComponent(venueTitle + ', ' + fullAddress)}`
  ...
  const scheduleItems = useMemo(() => {
    if (staticData?.events && staticData.events.length > 0) return staticData.events;
  ...
  const showRsvp = savedData
    ...
    : true // Force true for Rohit and Manpreet custom template
    ...
    <div className="relative min-h-screen bg-[#181311] text-[#4A3326] flex justify-center selection:bg-[#E8C29D]/40">
      ...
      <main className="relative w-full max-w-[480px] md:max-w-[820px] mx-auto bg-[#ECE3D1] shadow-[0_0_80px_rgba(0,0,0,0.7)] overflow-hidden flex flex-col">
        ...
        <RohitAndManpreetHero 
          ...
          fullAddress={"Chandigarh, Punjab, India"}
        />
        )}
        ...
        {/* ── SECTION 5: VENUE & NAVIGATION ── */}
        {showVenue && (
        <RohitAndManpreetVenue 
        ...
        {showRsvp && (
          <InviteQRSVP
            events={[]}
            hideEvents={true}
            weddingCode={savedData?.code || draftData?.code || staticData?.code}
            groupSlug={groupSlug}
            isPreview={!(savedData || staticData?.code)}
            theme="royal"
        ...
        {/* ── SECTION 9: LOVE AND REGARDS ── */}
        <RohitAndManpreetLoveAndRegards 
          ourPhotoBgMobile={storyBgMobile}
