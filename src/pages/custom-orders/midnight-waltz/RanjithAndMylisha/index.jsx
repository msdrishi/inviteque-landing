...
import MidnightWaltzCalendar from './components/MidnightWaltzCalendar.jsx'
...
//  MAIN PAGE EXPORT
// ═══════════════════════════════════════════════════════════════
export default function CustomMidnightWaltzRanjithAndMylisha({ groupSlug: propGroupSlug } = {}) {
  ...
  const calendarData = useMemo(() => {
    const year = parseInt(data.hero.weddingYear, 10)
    const month = new Date(`${data.hero.weddingMonth} 1, ${year}`).getMonth()
    const targetDate = parseInt(data.hero.weddingDate, 10)
    ...
    for (let i = 0; i < firstDayIndex; i++) {
      calendarDays.push({ day: "", isCurrent: false, isTarget: false })
    }
    for (let i = 1; i <= daysInMonth; i++) {
      calendarDays.push({
        day: i.toString(),
        isCurrent: true,
        isTarget: i === targetDate
      })
    }
    ...
    return {
      year,
      monthName,
      targetDateStr: `${data.hero.weddingDate} ${monthName} ${year}`,
      weekDays,
      calendarDays
    }
  }, [])
  ...
  <VenueMidnightWaltz
          ...
          <div className="w-full">
            <MidnightWaltzCalendar 
              calendarData={calendarData} 
              fullAddress={data.venue.venueName}
            />
          </div>
